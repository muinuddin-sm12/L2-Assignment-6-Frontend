...
// console.log(data)
  ...
  return (
    <div className="grid grid-cols-1 mx-auto w-full sm:grid-cols-2 md:grid-cols-3 gap-6 p-4">
      {data.map((meal, index) => (
        <div
          onClick={() => handleCardClick(meal?._id)}
          key={index}
          className="relative group overflow-hidden rounded-xl break-inside-avoid cursor-pointer shadow-md"
        >
          {/* Image */}
          <Image
            src={meal?.image!}
